...
**投资信号与资金流向（持续资金流向、主力资金为何一直无信号）**

- 「持续资金流向」「主力资金」等信号依赖表 **stex.moneyflow**（每日资金净流入、特大单等）。
- 该表**仅**在以下情况写入：
  - **采集跟踪股票数据**：对跟踪列表拉取约 2 年日线时，会一并拉取该区间的资金流向并写入；
  - **增量日线(最新)**：仅对**跟踪列表**补写当日（或指定起始日期起）的资金流向。
- 若从未执行「采集跟踪股票数据」，且只跑过「增量日线」且只有 1 天，则 moneyflow 最多只有 1 天；「持续资金流向」需要**连续 5 日**资金数据才会给出看涨/看跌，否则会显示无信号或中性。
- Tushare 资金流向接口（`moneyflow`）需要 **2000+ 积分**，积分不足时接口可能返回空或报错，导致无法写入。请先在 [Tushare 官网](https://tushare.pro) 确认积分与权限，再执行一次「采集跟踪股票数据」为跟踪股补齐历史资金流向。
